...
// TODO: This is a copy of the `fn run` because I did not find a way to decouple the whole logic
// from `ctx.recv()`.
async fn run_approval_on_worker_thread<
	B,
...
>(
	mut approval_work: futures::channel::mpsc::Receiver<FromOrchestra<ApprovalVotingMessage>>,
	mut sender: Sender,
	mut approval_voting_sender: AVSender,
	mut subsystem: ApprovalVotingSubsystem,
	assignment_criteria: Box<dyn AssignmentCriteria + Send + Sync>,
	mut backend: B,
) -> SubsystemResult<()>
	...
	B: Backend,
{
	...
	loop {
		...
		let actions = futures::select! {
			...
			next_msg = approval_work.select_next_some().fuse() => {
				let mut actions = handle_from_overseer(
					&mut sender,
					&mut approval_voting_sender,
					&subsystem.spawner,
...
pub async fn start_approval_worker<
	...
	AVSender: SubsystemSender<ApprovalDistributionMessage>,
>(
	approval_work: futures::channel::mpsc::Receiver<FromOrchestra<ApprovalVotingMessage>>,
	sender: Sender,
	approval_voting_sender: AVSender,
	config: Config,
	db: Arc<dyn Database>,
	keystore: Arc<LocalKeystore>,
	mode: Mode,
	...
	let sync_oracle = match mode {
		Mode::Active => todo!(),
		Mode::Syncing(oracle) => oracle,
	};
