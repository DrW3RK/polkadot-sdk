	...
	fn try_from(msg: ApprovalVotingParallelMessage) -> Result<Self, Self::Error> {
		match msg {
			ApprovalVotingParallelMessage::CheckAndImportAssignment(cert, bitfield, tranche) =>
				Ok(ApprovalVotingMessage::CheckAndImportAssignment(cert, bitfield, tranche)),
			ApprovalVotingParallelMessage::CheckAndImportApproval(vote) =>
				Ok(ApprovalVotingMessage::CheckAndImportApproval(vote)),
...
/// TODO: Add logic to enable it dynamically at node startup.
pub fn approval_voting_parallel_enabled() -> bool {
	false
}
